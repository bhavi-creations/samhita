        ...
        <div class="card-body">
            <div class="row">
                ...
                <div class="col-md-6">
                    ...
                    ?>
                    <p><strong>Sub Total:</strong> ₹<?= number_format($sales_order['sub_total'], 2) ?></p>
                    ...
                    <p><strong>Grand Total:</strong> ₹<?= number_format($sales_order['final_total_amount'], 2) ?></p>
    ...
    <div class="card mb-4">
        ...
        <div class="card-body">
            <div class="row">
                <div class="col-md-6">
                    <h5>Distributor</h5>
                    <p><strong>Agency Name:</strong> <?= esc($distributor['agency_name'] ?? 'N/A') ?></p>
                    <p><strong>Owner Name:</strong> <?= esc($distributor['owner_name'] ?? 'N/A') ?></p>
                    <p><strong>Owner Phone:</strong> <?= esc($distributor['owner_phone'] ?? 'N/A') ?></p>
                    <p><strong>Agency Address:</strong> <?= esc($distributor['agency_address'] ?? 'N/A') ?></p>
                    <p><strong>Agency GST Number:</strong> <?= esc($distributor['agency_gst_number'] ?? 'N/A') ?></p>
                </div>
                <div class="col-md-6">
                    <h5>Marketing Person</h5>
                    <p><strong>Name:</strong> <?= esc($marketing_person['name'] ?? 'N/A') ?></p>
                    <p><strong>Custom ID:</strong> <?= esc($marketing_person['custom_id'] ?? 'N/A') ?></p>
                    <p><strong>Email:</strong> <?= esc($marketing_person['email'] ?? 'N/A') ?></p>
                </div>
            </div>
        </div>
    </div>
    
    <!-- Updated section to display GST details in a table -->
    <?php if (!empty($gst_rates_details)): ?>
        <div class="card mb-4">
            <div class="card-header">
                <h4>Applied GST Rates</h4>
            </div>
            <div class="card-body">
                    ...
                    <table class="table table-striped table-bordered">
                        ...
                        <tbody>
                            <?php foreach ($payments as $payment): ?>
                                <tr>
                                    <td><?= esc(date('d-M-Y', strtotime($payment['created_at']))) ?></td>
