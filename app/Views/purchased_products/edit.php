...
<div class="container-fluid mt-4">
    <div class="card shadow-sm">
        <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
            <h4 class="mb-0"><?= esc($title) ?>: <span class="text-warning"><?= esc($purchasedProduct['name']) ?></span></h4>
        ...
        <div class="card-body">
            ...
            <form action="<?= base_url('purchased-products/update/' . $purchasedProduct['id']) ?>" method="post">
                <?= csrf_field() ?>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="name" id="name" class="form-control <?= (isset($validation) && $validation->hasError('name')) ? 'is-invalid' : '' ?>"
                           value="<?= old('name', $purchasedProduct['name']) ?>" required>
                ...
                <div class="mb-3">
                    ...
                    <textarea name="description" id="description" class="form-control" rows="3"><?= old('description', $purchasedProduct['description']) ?></textarea>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <select name="unit_id" id="unit_id" class="form-control <?= (isset($validation) && $validation->hasError('unit_id')) ? 'is-invalid' : '' ?>" required>
                        ...
                        <?php foreach ($units as $unit): ?>
                            <option value="<?= esc($unit['id']) ?>" <?= old('unit_id', $purchasedProduct['unit_id']) == $unit['id'] ? 'selected' : '' ?>>
                                <?= esc($unit['name']) ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
